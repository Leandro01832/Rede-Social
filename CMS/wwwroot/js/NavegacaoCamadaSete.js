$(document).ready(function() {
    ...
    $("#voltar").click(function() {
        ...
        window.location.href = "/SubSubGrupo/" + valorPaginaPadraoLink + "/" + indexSubStory + "/" + indexGrupo + "/" +
         indexSubGrupo + "/" + indexSubSubGrupo + "/" + indexCamadaSeis + "/" + indexCamadaSete + "/" + valorAnterior + "/" + auto + "/" + compartilhante;
    });

    $("#avancar").click(function() {
        redirecionar(valorProximo);
        window.location.href = "/SubSubGrupo/" + valorPaginaPadraoLink + "/" + indexSubStory + "/" + indexGrupo + "/" +
         indexSubGrupo + "/" + indexSubSubGrupo + "/" + indexCamadaSeis + "/" + indexCamadaSete + "/" + valorProximo + "/" + auto + "/" + compartilhante;
    });
        ...
        if ($("#automatico").is(':checked') == true) {
            ...
            setTimeout(function() {
                ...
                if (valorAtual < valorQuant){
                    ...
                    window.location.href = "/SubSubGrupo/" + valorPaginaPadraoLink + "/" + indexSubStory + "/" + indexGrupo +
                     "/" + indexSubGrupo + "/" + indexSubSubGrupo + "/" + indexCamadaSeis + "/" + indexCamadaSete +  "/1" + "/" + auto + "/" + compartilhante;
            }, tempo);
        }
        else
        window.location.href = "/SubSubGrupo/" + valorPaginaPadraoLink + "/" + indexSubStory + "/" + indexGrupo +
        "/" + indexSubGrupo + "/" + indexSubSubGrupo + "/" + indexCamadaSeis + "/" + indexCamadaSete + "/" + valorAtual + "/" + 0 + "/" + compartilhante;
    });
            ...
            data: { Indice: valorPaginaPadraoLink, IndiceSubStory : indexSubStory,
                 IndiceGrupo : indexGrupo, IndiceSubGrupo : indexSubGrupo,
                  IndiceSubSubGrupo : indexSubSubGrupo, IndiceCamadaSeis : indexCamadaSeis,
                   IndiceCamadaSete : indexCamadaSete }
        })
            .done(function(response) {
                $.cookie('automatico', '1');

                if(response[0] != 0)
                window.location.href = "/CamadaSete/" + response[0]  + "/" + response[1] + "/" + response[2] +
                 "/" + response[3] + "/" + response[4] + "/" + response[5] + "/" + response[6] + "/1/"  + auto + "/" + compartilhante;
                else
                window.location.href = "/CamadaSeis/" + valorPaginaPadraoLink + "/1/1/1/1/1/1/" + auto + "/" + compartilhante;
            });
    }
