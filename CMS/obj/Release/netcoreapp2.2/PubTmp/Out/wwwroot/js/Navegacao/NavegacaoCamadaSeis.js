$(document).ready(function() {
    ...
    $("#voltar").click(function() {
        ...
        window.location.href = "/SubSubGrupo/" + valorPaginaPadraoLink + "/" + indexSubStory + "/" + indexGrupo + "/" +
         indexSubGrupo + "/" + indexSubSubGrupo + "/" + indexCamadaSeis + "/" + valorAnterior + "/" + auto + "/" + compartilhante;
    });

    $("#avancar").click(function() {
        redirecionar(valorProximo);
        window.location.href = "/SubSubGrupo/" + valorPaginaPadraoLink + "/" + indexSubStory + "/" + indexGrupo + "/" +
         indexSubGrupo + "/" + indexSubSubGrupo + "/" + indexCamadaSeis + "/" + valorProximo + "/" + auto + "/" + compartilhante;
    });
        ...
        if ($("#automatico").is(':checked') == true) {
            ...
            setTimeout(function() {
                ...
                if (valorAtual < valorQuant){
                    ...
                    window.location.href = "/SubSubGrupo/" + valorPaginaPadraoLink + "/" + indexSubStory + "/" + indexGrupo +
                     "/" + indexSubGrupo + "/" + indexSubSubGrupo + "/" + indexCamadaSeis +  "/1" + "/" + auto + "/" + compartilhante;
            }, tempo);
        }
        else
        window.location.href = "/SubSubGrupo/" + valorPaginaPadraoLink + "/" + indexSubStory + "/" + indexGrupo +
        "/" + indexSubGrupo + "/" + indexSubSubGrupo + "/" + indexCamadaSeis + "/" + valorAtual + "/" + 0 + "/" + compartilhante;
    });
    ...
    function BuscarStory() {
        $.ajax({
            ...
            .done(function(response) {
                ...
                if(response[0] != 0)
                window.location.href = "/CamadaSeis/" + response[0]  + "/" + response[1] + "/" + response[2] +
                 "/" + response[3] + "/" + response[4] + "/" + response[5] + "/1/"  + auto + "/" + compartilhante;
                else
                window.location.href = "/SubSubGrupo/" + valorPaginaPadraoLink + "/1/1/1/1/1/" + auto + "/" + compartilhante;
            });
    }
