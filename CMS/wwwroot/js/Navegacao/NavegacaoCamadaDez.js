$(document).ready(function() {
    ...
    var indexCamadaSeis = $("#indexcamadaseis").val();
    var indexCamadaSete = $("#indexcamadasete").val();
    var indexCamadaOito = $("#indexcamadaoito").val();
    var indexCamadaNove = $("#indexcamadanove").val();
    var indexCamadaDez = $("#indexcamadadez").val();
    ...
    $("#voltar").click(function() {
        ...
        window.location.href = "/SubSubGrupo/" + valorPaginaPadraoLink + "/" + indexSubStory + "/" + indexGrupo + "/" +
         indexSubGrupo + "/" + indexSubSubGrupo + "/" + indexCamadaSeis + "/" + indexCamadaSete +
          "/" + indexCamadaOito + "/" + indexCamadaNove + "/" + indexCamadaDez + "/" + 
           valorAnterior + "/" + auto + "/" + compartilhante;
    });
    ...
    $("#avancar").click(function() {
        ...
        window.location.href = "/SubSubGrupo/" + valorPaginaPadraoLink + "/" + indexSubStory + "/" + indexGrupo + "/" +
         indexSubGrupo + "/" + indexSubSubGrupo + "/" + indexCamadaSeis + "/" + indexCamadaSete +
          "/" + indexCamadaOito + "/" + indexCamadaNove + "/" + indexCamadaDez +
            "/" + valorProximo + "/" + auto + "/" + compartilhante;
    });
        ...
        if ($("#automatico").is(':checked') == true) {
            ...
            setTimeout(function() {
                ...
                if (valorAtual < valorQuant){
                    redirecionar(valorProximo);
                    window.location.href = "/SubSubGrupo/" + valorPaginaPadraoLink + "/" + indexSubStory + "/" + indexGrupo +
                     "/" + indexSubGrupo + "/" + indexSubSubGrupo + "/" + indexCamadaSeis + "/" +
                      indexCamadaSete + "/" + indexCamadaOito + "/" + indexCamadaNove + "/" + indexCamadaDez + "/" + valorProximo + "/" + auto + "/" + compartilhante;
                }
                else
                    window.location.href = "/SubSubGrupo/" + valorPaginaPadraoLink + "/" + indexSubStory + "/" + indexGrupo +
                     "/" + indexSubGrupo + "/" + indexSubSubGrupo + "/" + indexCamadaSeis + "/" +
                      indexCamadaSete + "/" + indexCamadaOito + "/" + indexCamadaNove + "/" + indexCamadaDez +  "/1" + "/" + auto + "/" + compartilhante;
            }, tempo);
        }
        else
        window.location.href = "/SubSubGrupo/" + valorPaginaPadraoLink + "/" + indexSubStory + "/" + indexGrupo +
        "/" + indexSubGrupo + "/" + indexSubSubGrupo + "/" + indexCamadaSeis + "/" + indexCamadaSete +
         "/" + indexCamadaOito + "/" + indexCamadaNove + "/" + indexCamadaDez + "/" + valorAtual + "/" + 0 + "/" + compartilhante;
    });
    ...
    function BuscarStory() {
        $.ajax({
            ...
            data: { Indice: valorPaginaPadraoLink, IndiceSubStory : indexSubStory,
                 IndiceGrupo : indexGrupo, IndiceSubGrupo : indexSubGrupo,
                  IndiceSubSubGrupo : indexSubSubGrupo, IndiceCamadaSeis : indexCamadaSeis,
                   IndiceCamadaSete : indexCamadaSete, IndiceCamadaOito : indexCamadaOito,
                    IndiceCamadaNove : indexCamadaNove, IndiceCamadaDez : indexCamadaDez }
        })
            .done(function(response) {
                $.cookie('automatico', '1');

                if(response[0] != 0)
                window.location.href = "/CamadaDez/" + response[0]  + "/" + response[1] + "/" + response[2] +
                 "/" + response[3] + "/" + response[4] + "/" + response[5] + "/" + response[6] + "/" + response[7] +
                 "/" + response[8] + "/" + response[9] + "/1/"  + auto + "/" + compartilhante;
                else
                window.location.href = "/CamadaNove/" + valorPaginaPadraoLink + "/1/1/1/1/1/1/1/1/1/" + auto + "/" + compartilhante;
            });
    }
    ...
    if (auto == 1) {
        ...
        setTimeout(function() {
            ...
            if (valorAtual < valorQuant && auto == 1){
                redirecionar(valorProximo);
                window.location.href = "/SubSubGrupo/" + valorPaginaPadraoLink  +
                 "/" + indexSubStory +  "/" + indexGrupo  +
                 "/" + indexSubGrupo + "/" + indexSubSubGrupo +
                  "/" + indexCamadaSeis + "/" + indexCamadaSete + "/" + indexCamadaOito +
                  "/" + indexCamadaNove + "/" + indexCamadaDez +
                   "/"  + valorProximo + "/" + auto + "/" + compartilhante;
            }
            else if (auto == 1)             
             BuscarStory();          
